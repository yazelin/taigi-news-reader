...
結論：**正式 Item ID 已固定；`0.1.3` engineering／exact-package／Dashboard基線均已完成，既有`0.1.2` live-service回歸仍有效，operator也明確確認先前曝光的Groq／Gemini keys全部撤銷。2026-07-14 已取消舊`0.1.2`審查、上傳 exact `0.1.3`，並在Submit dialog取消「通過審查後自動發布」後成功提交；Status reload顯示「這個草稿尚待審查」。** 現在是Private／deferred／pending review，不是approved或published；review期間仍須維持backend與reviewer credential可用。
...
- `0.1.3` Manifest V3、`minimum_chrome_version=116`、name／version／description、source manifest／package／lockfile root version與production build均通過 release checks。
- Manifest 的 canonical RSA SPKI public key 推導出正式 CWS Item ID `nejhlfbnjkbdjcaaklaofggkikdlpakn`。`npm run release:check` 已重新解析 public key、驗證 RSA／canonical DER，並要求推導 ID 與 `package.json.cwsItemId` 完全相同；fresh Chrome profile 從 exact `0.1.3` ZIP 註冊後也觀察到同一 service-worker origin。
...
- `0.1.3` 的私人測試憑證是逐人 bearer invite token：extension raw token 存 `chrome.storage.local`、綁 configured origin，只送該 origin `/v1/`；server config 只有 SHA-256 digest＋stable pseudonymous subject。它是 CWS Authentication information，不是 provider key。Live server 已載入多個彼此獨立的 subjects；任何 evidence 都不得記 raw token 或 digest。
...
- MMS 將通過gate的POJ切成最多200字元的inference chunks：依序優先使用空白、連字號及不讓下一段以Unicode combining mark起頭的hard boundary，並停用tokenizer的二次normalization以保留段落交界。各段在同一single-flight worker與整份480秒deadline中產生bounded PCM，最後只輸出一個WAV；每段共用同一16 MiB總sample cap且sample rate必須一致。Transformers forward回傳tensor後仍先以`numel()`做pre-`.tolist()`檢查，但這與200字元input bound都不能宣稱阻止model內部所有配置峰值。Live profile另限600 source／6,000 translated characters、2 GiB memory/no-swap及4 CPUs。
...
- `0.1.3`公開分享素材另保存於`docs/screenshots/`，使用repo自有新聞fixture與空白Chrome設定檔，不含邀請碼、API key或第三方新聞圖文；它們已供README／Pages使用。本次只替換待審package，沒有宣稱Dashboard既有listing screenshot也已換成這批素材。
...
- 自動基線為 extension ESLint／production build／`96/96` tests，以及 backend `181 passed`。Backend新增anti-abuse regressions涵蓋send failure仍release delivery lease、concurrent DELETE retained-byte accounting、pending DELETE capacity、non-cooperative MMS timeout/cancel single-flight、200字元MMS inference chunking、單一bounded WAV、pre-`.tolist()` cap、strict direct disable、hosted translation異常重試與private-beta ingress／resource limits；NGINX 1.29.3兩組`nginx -t` harness及edge 403／429 CORS、method gate容器行為測試通過。POJ normalization仍只處理安全 typography，中文、數字、不支援拉丁字母及symbols fail closed。
- `deploy/private-beta/` 提供Internet-facing ingress／Compose override與rollback runbook，static tests鎖定600／6,000／16 MiB、200字元MMS inference chunks、480秒整份timeout、2 GiB/no-swap、4 CPU、single worker、direct 404、pinned CORS、source-IP limits及no request-body logging。2026-07-13 已將此profile套用到`.11`：backend沒有host port，durable quota database已掛載，effective resource limits與single-worker contract相符。
...
- 正式 CWS ID 的 exact `0.1.2` ZIP 曾在 fresh Chromium profile 註冊為 `nejhlfbnjkbdjcaaklaofggkikdlpakn`，通過原生 exact-origin optional permission、subject quota／UTC reset顯示、live Groq＋MMS playback與history。從history重播後，新增health及backend job requests均為0；這仍是live-service歷史回歸證據，但其待審草稿已由`0.1.3`取代。
- `0.1.3` exact ZIP 已解壓到原生Chrome fresh profile，runtime ID確認為正式`nejhlfbnjkbdjcaaklaofggkikdlpakn`，並以自有fixture通過真實工具列action：首次自動擷取exact tab，側欄保持開啟時跨origin導頁會清除舊預覽，側欄按鈕會提示重新授權，再按工具列圖示即可在不關側欄的情況下自動擷取新頁。測試沒有後端、provider、token或第三方素材；另產生可用於listing的1280×800讀取與空白設定頁截圖。這個exact package現已提交Private review。
...
2. 已把 public key 放入 source manifest 的 `key`，正式 ID 也已 pin 到 live edge／backend。`0.1.3` exact artifact、fresh-profile action E2E及Dashboard upload均已完成，package version／permissions已重載確認；`0.1.2`的live-service／external E2E仍作為歷史回歸證據。Public key可公開，但CWS signing private key／OAuth token不得放進repo。
...
4. Current exact ZIP 建檔時已確認 `src/manifest.json` 與 `package.json` version 都是 `0.1.3`；repo baseline為backend `181 passed`、extension `96/96`＋lint／build／release audit，`npm run package:store`通過。ZIP為52,670 bytes、SHA-256 `8ad5e78ec202524db1912d7e0148aeb05f4085eb437626b842581ea52a072e71`，原生Chrome exact-package／fresh-profile action換頁手測也已通過。2026-07-14已取消舊`0.1.2`審查、上傳同一`0.1.3` ZIP並以Private／deferred模式送審；仍不得宣稱approved或published。
5. `.11` single-worker backend、durable quota volume、nginx edge、600／6,000／16 MiB、200字元MMS inference chunks／480秒整份timeout、2 GiB/no-swap／4 CPU caps與非LAN smoke已完成；Groq ZDR／replacement key active與舊Groq／Gemini keys撤銷均由operator明確確認。持續保留monitoring／rollback readiness。
...
2. 把版本再提升為高於當時private build的版本（例如 `0.1.4`），重新 build、測試、打包、上傳並送審。不能只在 Dashboard 把 visibility 切 Public 而沿用未重新稽核的 credential／privacy 流程。
...
Effective beta limits verified (600/6000/16 MiB; 2 GiB/no-swap; 4 CPU):
