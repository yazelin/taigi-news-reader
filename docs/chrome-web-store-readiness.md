...
- MMS 在 Transformers forward 回傳 tensor後、呼叫 `.tolist()` 前先以 `numel()` 套 audio sample cap，WAV encoder也做 iterable／bytes cap；這避免超限 waveform再膨脹成Python list，但不能阻止 model forward本身先配置tensor。Live profile另限600 source／6,000 translated characters、16 MiB audio、2 GiB memory/no-swap及4 CPUs。
...
5. `.11` single-worker backend、durable quota volume、nginx edge、600／6,000／16 MiB、2 GiB/no-swap／4 CPU caps與非LAN smoke已完成；Groq ZDR／replacement key active與舊Groq／Gemini keys撤銷均由operator明確確認。持續保留monitoring／rollback readiness。
