...
結論：**正式 Item ID 已固定；`0.1.2` source、automated checks 與 exact ZIP 工程基線已完成，但該 ZIP 尚未上傳，private-beta ingress 範本尚未部署到 `.11`，外部 reviewer E2E、Groq ZDR 與 provider key rotation 也未完成，因此仍不可按 Submit for Review。** Dashboard 提供的 public key 已放入 manifest，Chrome 對 `0.1.1` 已確認同一正式 ID；`0.1.2` package identity 仍須用 fresh profile 重跑。更新草稿不等於送審或發佈。
...
| P0 | `0.1.2` external exact-package E2E 尚未完成 | Backend `164 passed`、extension `82/82`＋lint／build、package checks 與 exact ZIP hash已完成；仍須用 fresh profile 從該 ZIP 驗證正式 ID、正確 token、錯誤／撤銷 token、origin switch、401／429、quota display、terminal delivery／DELETE、STOP 及 replay zero-request，並從 operator LAN 外重跑 reviewer flow。 |
...
| P0 | Reviewer credential／submission | Dashboard 已儲存 Private distribution，publisher account 已加入 trusted testers，聯絡信箱也已驗證；不再把這三項列為未完成。仍須在 live endpoint完成後，透過 Dashboard 安全 reviewer credential 欄提供一組有期限／足額 quota 的個別 token，更新 test instructions並上傳 `0.1.2`。不得把共用 token 放進 extension、listing、repo、screenshot或公開 issue。 |
...
- Manifest 的 canonical RSA SPKI public key 推導出正式 CWS Item ID `nejhlfbnjkbdjcaaklaofggkikdlpakn`。`npm run release:check` 已重新解析 public key、驗證 RSA／canonical DER，並要求推導 ID 與 `package.json.cwsItemId` 完全相同；隔離 Chrome 對 `0.1.1` 已觀察到同一 service-worker origin，`0.1.2` exact ZIP 的 fresh-profile registration仍須重跑。
...
- Strict token mode 若同時設定 `TAIGI_ALLOW_DIRECT_SYNTHESIS=true`，backend 會啟動 fail closed。Private-beta nginx 只暴露 access 與 async job routes，direct route 固定 404；已測的 ingress 範本另有每 IP rate／connection limits 與 global connection cap。Job registry 仍是 process-local，所以 private beta 限定單 worker／replica。
- MMS 在 Transformers forward 回傳 tensor後、呼叫 `.tolist()` 前先以 `numel()` 套 audio sample cap，WAV encoder也做 iterable／bytes cap；這避免超限 waveform再膨脹成Python list，但不能阻止 model forward本身先配置tensor。範本因此另限600 source／2,000 translated characters、16 MiB audio、2 GiB memory/no-swap及4 CPUs。這些是repo範本／tests，不是`.11` live狀態。
...
- 自動基線為 extension ESLint／production build／`82/82` tests，以及 backend `164 passed`。Backend新增anti-abuse regressions涵蓋send failure仍release delivery lease、concurrent DELETE retained-byte accounting、pending DELETE capacity、non-cooperative MMS timeout/cancel single-flight、pre-`.tolist()` cap、strict direct disable與private-beta ingress／resource limits；NGINX 1.29.3兩組`nginx -t` harness通過。POJ normalization仍只處理安全 typography，中文、數字、不支援拉丁字母及symbols fail closed。
- `deploy/private-beta/` 提供Internet-facing ingress／Compose override與rollback runbook，已由static tests鎖定600／2,000／16 MiB、2 GiB/no-swap、4 CPU、single worker、direct 404、pinned CORS、source-IP limits及no request-body logging。這個目錄明確是**template，不是部署紀錄**；`.11`目前仍運行LAN-only基線。
...
2. 已把 public key 放入 source manifest 的 `key`，正式 ID 也已加入 edge／backend 過渡期 allowlist。Dashboard 目前仍是 `0.1.1`；`0.1.2` exact artifact 已形成並通過 automated package checks，但尚未上傳或完成 fresh-profile／external E2E。Public key 可公開，但 CWS signing private key／OAuth token 不得放進 repo。
...
4. Repo 階段已確認 `src/manifest.json` 與 `package.json` version 都是 `0.1.2`，backend `164 passed`、extension `82/82`＋lint／build與`npm run package:store`通過；固定 artifact bytes／hash如上。任何後續 source改動都要bump version或至少重建、重算hash並重跑同級checks。
5. 完成上表營運P0：Groq ZDR與provider key rotation，依private-beta範本部署single-worker backend＋durable quota volume、nginx per-IP limits、600／2,000／16 MiB及2 GiB／4 CPU caps、production logging／monitoring與rollback。從operator LAN外驗證；不要因範本/tests存在就宣稱live。
6. 解壓exact ZIP到空目錄load unpacked，重跑[manual-test.md](manual-test.md)與[listing reviewer steps](chrome-web-store-listing.md#reviewer-test-instructions)，記錄fresh-profile正式ID、token／quota／delivery／STOP／replay證據、endpoint health identity及外部E2E。
