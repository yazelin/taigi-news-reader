...
- `0.1.2` 的私人測試憑證是逐人 bearer invite token：extension raw token 存 `chrome.storage.local`、綁 configured origin，只送該 origin `/v1/`；server config 只有 SHA-256 digest＋stable pseudonymous subject。它是 CWS Authentication information，不是 provider key。Live server 已載入多個彼此獨立的 subjects；任何 evidence 都不得記 raw token 或 digest。
...
- 下一版公開分享素材另保存於`docs/screenshots/`，使用repo自有新聞fixture與空白Chrome設定檔，不含邀請碼、API key或第三方新聞圖文；它們可供README／Pages使用，尚未取代目前Dashboard已提交的`0.1.2`素材。
...
- `0.1.3` exact ZIP 已解壓到原生Chrome fresh profile，runtime ID確認為正式`nejhlfbnjkbdjcaaklaofggkikdlpakn`，並以自有fixture通過真實工具列action：首次自動擷取exact tab，側欄保持開啟時跨origin導頁會清除舊預覽，側欄按鈕會提示重新授權，再按工具列圖示即可在不關側欄的情況下自動擷取新頁。測試沒有後端、provider、token或第三方素材；另產生可用於下一版listing的1280×800讀取與空白設定頁截圖。
...
4. Exact ZIP 建檔當時已確認 `src/manifest.json` 與 `package.json` version 都是 `0.1.2`，backend `166 passed`、extension `82/82`＋lint／build與`npm run package:store`通過；固定 artifact bytes／hash如上。其後backend-only修正不會改變已提交ZIP；目前repo baseline是backend `181 passed`，另有已升版但尚未上傳的extension `0.1.3` candidate，`96/96`＋lint／build／release audit通過，ZIP為52,670 bytes、SHA-256 `8ad5e78ec202524db1912d7e0148aeb05f4085eb437626b842581ea52a072e71`，原生Chrome exact-package／fresh-profile action換頁手測也已通過。它尚未取代現有submission，不得宣稱已送審。
