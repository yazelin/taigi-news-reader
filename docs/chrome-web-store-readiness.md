...
- 自動基線為 extension ESLint／production build／`87/87` tests，以及 backend `177 passed`。Backend新增anti-abuse regressions涵蓋send failure仍release delivery lease、concurrent DELETE retained-byte accounting、pending DELETE capacity、non-cooperative MMS timeout/cancel single-flight、200字元MMS inference chunking、單一bounded WAV、pre-`.tolist()` cap、strict direct disable與private-beta ingress／resource limits；NGINX 1.29.3兩組`nginx -t` harness及edge 403／429 CORS、method gate容器行為測試通過。POJ normalization仍只處理安全 typography，中文、數字、不支援拉丁字母及symbols fail closed。
...
4. Exact ZIP 建檔當時已確認 `src/manifest.json` 與 `package.json` version 都是 `0.1.2`，backend `166 passed`、extension `82/82`＋lint／build與`npm run package:store`通過；固定 artifact bytes／hash如上。其後backend-only的MMS修正不會改變已提交ZIP；目前repo baseline是backend `177 passed`、extension `87/87`＋lint／build。任何後續extension source改動都要bump version或至少重建、重算hash並重跑同級checks。
