...
| P0 | `0.1.2` external exact-package E2E 尚未完成 | Backend `166 passed`、extension `82/82`＋lint／build、package checks 與 exact ZIP hash已完成；仍須用 fresh profile 從該 ZIP 驗證正式 ID、正確 token、錯誤／撤銷 token、origin switch、401／429、quota display、terminal delivery／DELETE、STOP 及 replay zero-request，並從 operator LAN 外重跑 reviewer flow。 |
