...
- Backend：`92 passed`，涵蓋 API schema、CORS、provider 錯誤、hosted adapters、Gemini、GPT-OSS reasoning、POJ repair、MMS WAV，以及 async job 的 UUID4、狀態轉換、TTL、active cap、DELETE／shutdown cancellation 與不保存原文。Strict request tests 另驗證 `/v1/` missing／wrong extension header 為 403、正確 header 可接受沒有 Origin 的 GET poll、存在時 header／Origin 必須同一 ID，以及 preflight 可協商 `x-taigi-extension-id`。LAN deployment test 也鎖定 local MMS image 必須先從 PyTorch 官方 CPU index 安裝 Torch，再安裝 TTS extras，避免純 CPU 主機誤帶 CUDA runtime。
