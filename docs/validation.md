...
- Extension 的 71-test automated baseline，以及真實 Chromium 的 strict-header async job、default-off、opt-in／完整 queue save、離線 START／REPLAY cache hit、corrupt START 零 request／雙向清除與 clear-all IndexedDB E2E 已通過。正式 manifest 也已通過原生 optional-permission prompt、真實 Groq＋MMS 播放完成、history 與零 request replay。Schema-v2 provider fingerprint 的 provider-switch、LRU／TTL、逐筆刪除與停用即清除已有自動／fault-injection 驗證，仍須依 [manual-test.md](manual-test.md) 在目標長輩裝置做可用性與更多手動驗收。
- `.11` 的非商用 MMS hosted backend、HTTPS root reverse proxy、LAN allowlist、固定 unpacked extension ID gate、rate／connection limit與不記 request body 的基線已部署，短句與 80 字單段新聞都已通過 production job。仍須完成多分段長文／並發／持續監控與備份演練；模型維持 CC BY-NC 4.0 限制，若未來改成商用才可換 provider／另取授權。
- 目前 endpoint 刻意只限 LAN；外部 CWS reviewer／一般公網使用者仍無法使用。Extension header／Origin 都可偽造，不是 authentication；送審或開公網前必須另加真正 authentication／abuse controls與 reviewer path，不能只移除 nginx `deny all`。
