...
┌─────────────────────────────────────────────┐
│ Chrome                                      │
│  新聞頁 → content extraction → side panel   │
└──────────────────┬──────────────────────────┘
                   │ 純文字；使用者主動觸發
                   ▼
┌─────────────────────────────────────────────┐
...
- `POST /v1/synthesize`：驗證輸入、呼叫 translator 和 synthesizer，回傳可播放音訊。
...
目前 `POST /v1/synthesize` 的 request 為：
...
`source_language` / `target_language` 是固定值，`rate` 範圍為 0.5–1.5。response 包含 `taigi_text`、`audio_base64`、`mime_type` 與 `provider`；音訊不得以 data URL 前綴混入 `audio_base64` 欄位。
