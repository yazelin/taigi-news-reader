...
- `DELETE /v1/synthesis-jobs/{job_id}`：只有 owner subject 能操作。Terminal payload 未在傳送時會直接移除；delivery lease 仍在傳送時只標記 delete-requested，等 response finalizer 釋放 bytes 後才移除。Pending job 會立刻對 owner 隱藏並讓重複 DELETE 維持 204，但若 provider coroutine／thread 不能安全取消，仍占用 active／outstanding capacity 到實際結束。Chrome 的 STOP 使用相同操作。
- `POST /v1/synthesize`：只保留給預設 local development／診斷；Chrome 正常路徑不維持這個長請求。Strict access-token mode 若同時設定 `TAIGI_ALLOW_DIRECT_SYNTHESIS=true`，backend 會啟動失敗；private-beta ingress 對 direct route 固定回 404。
...
STOP 會中止 client polling並 DELETE job。DELETE 先讓 owner 看不到 job，但不會謊稱已釋放實際 provider 容量：可合作取消的 coroutine 可結束；已進入 thread 的 MMS 等 non-cooperative work 仍維持 capacity charge，直到真正返回後丟棄 result。Backend shutdown 會要求取消 active tasks，再等待 detached MMS workers 實際結束後關閉 provider clients。
...
- Terminal result（完整合成結果或安全錯誤）只能由 owner GET 一次。GET claim 一個 opaque delivery lease 並先阻止第二次 GET；payload／error 與 retained-byte accounting 保留到 `FinalizingJSONResponse` 將 body 送完，或 send 失敗進入 `finally` 後，才由 background finalizer 釋放並留下 tombstone。傳送期間 DELETE 只設定 acknowledgement，不會提早清 bytes。未讀取 terminal、tombstone 與 lease 的 TTL 預設 600 秒；active lease 以自己的起始時間計算，後續 job API 操作會清除真正過期資料。
...
- DELETE 不等於立刻返還 capacity：discarded pending job 到 provider 真正結束前仍算 active／outstanding；送出中的 terminal payload到 delivery finalizer 前仍算 retained bytes。這使 repeated create／DELETE 與 slow／disconnected response 不能繞過 admission caps。
...
Private beta只暴露async job routes，並把單次限制縮為600 source characters、6,000 translated characters及16 MiB encoded WAV；每subject每UTC日20 jobs／12,000 characters，全域100 jobs／60,000 characters；container另有2 GiB hard memory/no-swap與4-CPU quota。這些值由[`deploy/private-beta/`](../deploy/private-beta/README.md)定義並於2026-07-13套用到`.11` live profile；16 MiB仍不能換算成固定音訊秒數。
